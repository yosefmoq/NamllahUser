...
class MainApiImpl @Inject constructor(
    ...
    fun getServiceProviders(lat:Double,lng:Double,service_id:Int):Maybe<ServiceProviderResponse> = Maybe.create {
        val response = mainApi.getNearServiceProviders(serviceId = service_id,lng = lng,lat = lat).execute()
        if(response.isSuccessful){
            val gson = GsonBuilder().create()
    ...
    fun getNotification():Maybe<NotificationResponse> = Maybe.create{
        ...
        if(response.isSuccessful){
            ...
            val str = JSONObject(response.body()!!.string()).toString()
            val notificationResponse:NotificationResponse =  gson.fromJson(str,NotificationResponse::class.java)
    ...
    fun changePassword(oldPassword:String,password:String):Maybe<SignInResponse> = Maybe.create{
